...
//[[Rcpp::export]]
List effScoreFam(double beta, double sigma2, std::vector<std::string> fam_group,
                 Rcpp::NumericVector i_at, Rcpp::NumericVector i_dt,
                 Rcpp::NumericVector i_Delta, Rcpp::NumericVector i_G,
  ...
  */
  ...
  for (int k = 0; k < m; k++) {
    ...
    if (famsize[k] >= 1) {
      Cuhre(famsize[k], NCOMP, Integrand, 1e-6, 1e-12, 0, 0, 50000, 0,
    ...
    //	w, &ll, &error);
            // compute the partial derivative of likelihood regarding to
    \sigma^2. F.function = &fsigma; gsl_integration_qag (&F,-30*sqrt(sigma2),
                30*sqrt(sigma2), 1e-6, 1e-4, 10000,1,
                            w, &dfsigma, &error);
...
*/
  ...
  return List::create(Named("LogLik") = l_i, Named("beta_score") = betaScore,
