...
geneStat <- function(x, Z=NULL, GenABEL.data=NULL, alpha, 
                     theta=NULL, gtime, delta, beta=0, nCores=1,
										 ...
										 geneSet)
{
  ...
  if (class(GenABEL.data)[1] == "gwaa.data") {
    if (!is.null(x)) {
  ...
  # transfer alphas into gammas and prevent numerical 
  # crash by setting zero to 10^-8
  alpha[which(alpha[1:ntps] < 10^-8)] <- 10^-8
  alpha[1:ntps] <- log(-log(alpha[1:ntps]))
  
	# make it one vector for the nuisance parameter
  ...
  for (i in seq_len(length(geneSet))) {
    subSet_X <- as.matrix(x[, geneSet[[i]][[1]]], 
                          ncol=length(geneSet[[i]][[1]]))
    
    # compute the efficient score by c++ function
    Uij <- .Call(
      "_groupedSurv_effScore", 
      PACKAGE = "groupedSurv", 
      beta, 
      Params, 
      subSet_X, 
