  ...
  \begin{itemize}
	\item This document provides examples demonstrating how to use the
		...
		and compute the efficient score statistic, as well as gene-level statistics for grouped survival models. 
			...
			analyze grouped time-to-event data accounting for family structure of
...
% \scriptsize
  ...
  \begin{itemize}
    \item The input data is assumed to be organized as a \df{}, with rows 
  ...
  \end{itemize}
  Data with family structure~\cite{ripatti2004}
  \begin{itemize}
	\item The input \df{} is assumed to be organized such that records for each
...
@
Functions for data with family structure\\
<<with, eval=FALSE>>=
...
matrix, \texttt{Z}, are required as well. Optionally, users can specify the
...
it requires a matrix of variables to be tested for association with the outcome,
...
of interest. If the model includes covariates, the \texttt{Z} matrix is also
required. In the context of GWAS, covariates and variables of interest can be
provided as part of a GenABEL object (\texttt{GenABEL.data}), with the arguments
\texttt{x} and \texttt{Z} instead used to specify to the columns of
\texttt{GenABEL.data} to be included in the analyses.\\
\end{frame}
...
head(eff)
@
Or, genotype dosage data can be imported from a VCF file using the \texttt{BEDMatrix} package (after converting to a .bed file using PLINK~\cite{plink}).
